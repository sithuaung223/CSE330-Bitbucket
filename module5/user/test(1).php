...
<body>
  <ul class="year">
    <button id= "prev_year_btn" class="btn btn-primary"> Previous Year </button>
    <button id= "next_year_btn" class="btn btn-primary"> Next Year </button>
  </ul>
	<ul class="month">
      ...
      <div class="modal-dialog">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <input type="hidden" id="token" value='<?php session_start(); echo $_SESSION["token"];?>'/>
          </div>
...
function viewEvent(){
      ...
      xmlHttp.addEventListener("load", function(event){
        var jsonData = JSON.parse(event.target.responseText); // parse the JSON into a JavaScript object
        if(jsonData.success){
            $("#list_event").append('<li>'+jsonData.output[0]['date']+'</li>');
        }
        else{
